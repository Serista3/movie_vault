...
export default function PersonCard({ person, className }: PersonCardProps) {
  ...
  const wrapperClass = cn(
    `flex flex-col gap-2 items-start w-40 hover:scale-102 transition-all duration-300
    bg-secondary-dark`, 
    className
);
    ...
    <Card className={wrapperClass}>
        ...
        <Image 
            containerClassName="h-50 rounded-b-none"
            src={personImg} 
            alt={`Image of ${personName}`} 
        />

        {/* --- PERSON INFO --- */}
        <div className="flex flex-col gap-1.25 p-4">
            <Heading level={3} className="z-3 line-clamp-1"> 
                {personName}
            </Heading>
            <Paragraph className="text-tertiary-dark line-clamp-1 leading-5 z-3">
