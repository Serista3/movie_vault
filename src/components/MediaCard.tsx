...
export default function MediaCard({ media }: { media: MediaSummary }) {
  const mediaRating = Math.round(('vote_average' in media ? media.vote_average : 0) / 10 * 100);
      ...
      <div className="flex flex-col gap-1 text-left w-full">
        <Link to={`/media/${media.id}`}>
          <h3 className="text-base font-semibold text-white-light hover:text-main-light transition-colors duration-300 cursor-pointer">
            {'title' in media ? media.title : 'original_name' in media ? media.original_name : ''}
          </h3>
        </Link>
        <p className="text-sm font-light text-white-dark">
          {formatDateToReadable('release_date' in media ? media.release_date : 'first_air_date' in media ? media.first_air_date : '')}
        </p>
      </div>
